    ...
    namespace tools {
        namespace converter {
            ...
            public:
                ...
                void convert_16bit_depth(rs2::video_frame& depth)
                {
                        ...
                        [this, &frameset] {
                            for (size_t i = 0; i < frameset.size(); i++) {
                                ...
                                if (frame && (_streamType == rs2_stream::RS2_STREAM_ANY || frame.get_profile().stream_type() == _streamType)) {
                                    ...
                                    // // no depth colorization
                                    if (frame.get_profile().stream_type() == rs2_stream::RS2_STREAM_DEPTH) {
                                        // frame = _colorizer.process(frame);
                                    }
                                    std::cout << "[" << frame.get_width() << " " << frame.get_height() << " " << frame.get_bytes_per_pixel() << " " << frame.get_stride_in_bytes() << "]";
